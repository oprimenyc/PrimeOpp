// @primeopp-marketplace/test-goat
// TEST-ONLY adapter stub for GOAT. NO LIVE CONNECTIVITY.
// This adapter simulates GOAT marketplace behavior for local development,
...
function fakeEvidenceRecord(tenantId: string, kind: string, description: string, subjectId: string, payload: Readonly<Record<string, unknown>>): EvidenceRecord {
  return {
    ...
    kind,
    description,
    actor: { actorType: 'adapter', actorId: 'test-goat', tenantId },
    subject: { type: 'listing', id: subjectId },
    payload
  } as unknown as EvidenceRecord;
}

export class GOATTestAdapter implements MarketplaceChannelAdapter {
  readonly adapterId = 'test-goat_adapter';
  readonly version = '0.1.0-test';
  readonly channelId = 'test-goat';
  readonly manifest: ChannelManifest = getManifest('test-goat')!;
  ...
  async healthCheck(): Promise<{ healthy: boolean; message?: string }> {
    return { healthy: true, message: 'GOAT TEST-ONLY adapter healthy' };
  }
  ...
  async publishListing(listing: CanonicalListing): Promise<{ channelListingId: string; evidence: EvidenceRecord }> {
    const channelListingId = newId('goat_list');
    return {
      channelListingId,
      evidence: fakeEvidenceRecord(listing.tenantId, 'test_listing_published', `[TEST-ONLY GOAT] published: ${listing.title}`, listing.listingId, { channelListingId })
    };
  }

  async updateListing(channelListingId: string, listing: CanonicalListing): Promise<{ updated: boolean; evidence: EvidenceRecord }> {
    return { updated: true, evidence: fakeEvidenceRecord(listing.tenantId, 'test_listing_updated', `[TEST-ONLY GOAT] updated ${channelListingId}`, channelListingId, {}) };
  }

  async pauseListing(channelListingId: string): Promise<{ paused: boolean; evidence: EvidenceRecord }> {
    return { paused: true, evidence: fakeEvidenceRecord('', 'test_listing_paused', `[TEST-ONLY GOAT] paused ${channelListingId}`, channelListingId, {}) };
  }

  async resumeListing(channelListingId: string): Promise<{ resumed: boolean; evidence: EvidenceRecord }> {
    return { resumed: true, evidence: fakeEvidenceRecord('', 'test_listing_resumed', `[TEST-ONLY GOAT] resumed ${channelListingId}`, channelListingId, {}) };
  }

  async endListing(channelListingId: string): Promise<{ ended: boolean; evidence: EvidenceRecord }> {
    return { ended: true, evidence: fakeEvidenceRecord('', 'test_listing_ended', `[TEST-ONLY GOAT] ended ${channelListingId}`, channelListingId, {}) };
  }
  ...
  async syncInventory(channelListingId: string, quantity: number): Promise<{ synced: boolean; evidence: EvidenceRecord }> {
    return { synced: true, evidence: fakeEvidenceRecord('', 'test_inventory_synced', `[TEST-ONLY GOAT] synced qty=${quantity}`, channelListingId, { quantity }) };
  }

  async syncPrice(channelListingId: string, price: Money): Promise<{ synced: boolean; evidence: EvidenceRecord }> {
    return { synced: true, evidence: fakeEvidenceRecord('', 'test_price_synced', `[TEST-ONLY GOAT] synced price=${price.amount}`, channelListingId, { price }) };
  }
  ...
  async respondToOffer(offerId: string, response: 'accept' | 'decline' | 'counter', counterAmount?: Money): Promise<{ responded: boolean; evidence: EvidenceRecord }> {
    ...
    return { responded: true, evidence: fakeEvidenceRecord('', 'test_offer_responded', `[TEST-ONLY GOAT] responded`, offerId, {}) };
  }
  ...
  async acknowledgeOrder(channelOrderId: string): Promise<{ acknowledged: boolean; evidence: EvidenceRecord }> {
    return { acknowledged: true, evidence: fakeEvidenceRecord('', 'test_order_acknowledged', `[TEST-ONLY GOAT] ack ${channelOrderId}`, channelOrderId, {}) };
  }

  async cancelOrder(channelOrderId: string, reason: string): Promise<{ cancelled: boolean; evidence: EvidenceRecord }> {
    return { cancelled: true, evidence: fakeEvidenceRecord('', 'test_order_cancelled', `[TEST-ONLY GOAT] cancel ${channelOrderId}: ${reason}`, channelOrderId, { reason }) };
  }
  ...
  async verifyListing(channelListingId: string): Promise<{ verified: boolean; evidence: EvidenceRecord }> {
    return { verified: true, evidence: fakeEvidenceRecord('', 'test_listing_verified', `[TEST-ONLY GOAT] verify ${channelListingId}`, channelListingId, {}) };
  }

  async verifyOrder(channelOrderId: string): Promise<{ verified: boolean; evidence: EvidenceRecord }> {
    return { verified: true, evidence: fakeEvidenceRecord('', 'test_order_verified', `[TEST-ONLY GOAT] verify ${channelOrderId}`, channelOrderId, {}) };
  }

  async shutdown(): Promise<void> {}
}

export function createGOATTestAdapter(): GOATTestAdapter {
  return new GOATTestAdapter();
}
