// @primeopp-marketplace/test-walmart
// TEST-ONLY adapter stub for Walmart. NO LIVE CONNECTIVITY.
// This adapter simulates Walmart marketplace behavior for local development,
...
function fakeEvidenceRecord(tenantId: string, kind: string, description: string, subjectId: string, payload: Readonly<Record<string, unknown>>): EvidenceRecord {
  return {
    ...
    kind,
    description,
    actor: { actorType: 'adapter', actorId: 'test-walmart', tenantId },
    subject: { type: 'listing', id: subjectId },
    payload
  } as unknown as EvidenceRecord;
}

export class WMRTTestAdapter implements MarketplaceChannelAdapter {
  readonly adapterId = 'test-walmart_adapter';
  readonly version = '0.1.0-test';
  readonly channelId = 'test-walmart';
  readonly manifest: ChannelManifest = getManifest('test-walmart')!;
  ...
  async healthCheck(): Promise<{ healthy: boolean; message?: string }> {
    return { healthy: true, message: 'Walmart TEST-ONLY adapter healthy' };
  }
  ...
  async publishListing(listing: CanonicalListing): Promise<{ channelListingId: string; evidence: EvidenceRecord }> {
    const channelListingId = newId('wmrt_list');
    return {
      channelListingId,
      evidence: fakeEvidenceRecord(listing.tenantId, 'test_listing_published', `[TEST-ONLY Walmart] published: ${listing.title}`, listing.listingId, { channelListingId })
    };
  }

  async updateListing(channelListingId: string, listing: CanonicalListing): Promise<{ updated: boolean; evidence: EvidenceRecord }> {
    return { updated: true, evidence: fakeEvidenceRecord(listing.tenantId, 'test_listing_updated', `[TEST-ONLY Walmart] updated ${channelListingId}`, channelListingId, {}) };
  }

  async pauseListing(channelListingId: string): Promise<{ paused: boolean; evidence: EvidenceRecord }> {
    return { paused: true, evidence: fakeEvidenceRecord('', 'test_listing_paused', `[TEST-ONLY Walmart] paused ${channelListingId}`, channelListingId, {}) };
  }

  async resumeListing(channelListingId: string): Promise<{ resumed: boolean; evidence: EvidenceRecord }> {
    return { resumed: true, evidence: fakeEvidenceRecord('', 'test_listing_resumed', `[TEST-ONLY Walmart] resumed ${channelListingId}`, channelListingId, {}) };
  }

  async endListing(channelListingId: string): Promise<{ ended: boolean; evidence: EvidenceRecord }> {
    return { ended: true, evidence: fakeEvidenceRecord('', 'test_listing_ended', `[TEST-ONLY Walmart] ended ${channelListingId}`, channelListingId, {}) };
  }
  ...
  async syncInventory(channelListingId: string, quantity: number): Promise<{ synced: boolean; evidence: EvidenceRecord }> {
    return { synced: true, evidence: fakeEvidenceRecord('', 'test_inventory_synced', `[TEST-ONLY Walmart] synced qty=${quantity}`, channelListingId, { quantity }) };
  }

  async syncPrice(channelListingId: string, price: Money): Promise<{ synced: boolean; evidence: EvidenceRecord }> {
    return { synced: true, evidence: fakeEvidenceRecord('', 'test_price_synced', `[TEST-ONLY Walmart] synced price=${price.amount}`, channelListingId, { price }) };
  }
  ...
  async respondToOffer(offerId: string, response: 'accept' | 'decline' | 'counter', counterAmount?: Money): Promise<{ responded: boolean; evidence: EvidenceRecord }> {
    ...
    return { responded: true, evidence: fakeEvidenceRecord('', 'test_offer_responded', `[TEST-ONLY Walmart] responded`, offerId, {}) };
  }
  ...
  async acknowledgeOrder(channelOrderId: string): Promise<{ acknowledged: boolean; evidence: EvidenceRecord }> {
    return { acknowledged: true, evidence: fakeEvidenceRecord('', 'test_order_acknowledged', `[TEST-ONLY Walmart] ack ${channelOrderId}`, channelOrderId, {}) };
  }

  async cancelOrder(channelOrderId: string, reason: string): Promise<{ cancelled: boolean; evidence: EvidenceRecord }> {
    return { cancelled: true, evidence: fakeEvidenceRecord('', 'test_order_cancelled', `[TEST-ONLY Walmart] cancel ${channelOrderId}: ${reason}`, channelOrderId, { reason }) };
  }
  ...
  async verifyListing(channelListingId: string): Promise<{ verified: boolean; evidence: EvidenceRecord }> {
    return { verified: true, evidence: fakeEvidenceRecord('', 'test_listing_verified', `[TEST-ONLY Walmart] verify ${channelListingId}`, channelListingId, {}) };
  }

  async verifyOrder(channelOrderId: string): Promise<{ verified: boolean; evidence: EvidenceRecord }> {
    return { verified: true, evidence: fakeEvidenceRecord('', 'test_order_verified', `[TEST-ONLY Walmart] verify ${channelOrderId}`, channelOrderId, {}) };
  }

  async shutdown(): Promise<void> {}
}

export function createWMRTTestAdapter(): WMRTTestAdapter {
  return new WMRTTestAdapter();
}
