// @primeopp-marketplace/test-stockx
// TEST-ONLY adapter stub for StockX. NO LIVE CONNECTIVITY.
// This adapter simulates StockX marketplace behavior for local development,
...
function fakeEvidenceRecord(tenantId: string, kind: string, description: string, subjectId: string, payload: Readonly<Record<string, unknown>>): EvidenceRecord {
  return {
    ...
    kind,
    description,
    actor: { actorType: 'adapter', actorId: 'test-stockx', tenantId },
    subject: { type: 'listing', id: subjectId },
    payload
  } as unknown as EvidenceRecord;
}

export class STKXTestAdapter implements MarketplaceChannelAdapter {
  readonly adapterId = 'test-stockx_adapter';
  readonly version = '0.1.0-test';
  readonly channelId = 'test-stockx';
  readonly manifest: ChannelManifest = getManifest('test-stockx')!;
  ...
  async healthCheck(): Promise<{ healthy: boolean; message?: string }> {
    return { healthy: true, message: 'StockX TEST-ONLY adapter healthy' };
  }
  ...
  async publishListing(listing: CanonicalListing): Promise<{ channelListingId: string; evidence: EvidenceRecord }> {
    const channelListingId = newId('stkx_list');
    return {
      channelListingId,
      evidence: fakeEvidenceRecord(listing.tenantId, 'test_listing_published', `[TEST-ONLY StockX] published: ${listing.title}`, listing.listingId, { channelListingId })
    };
  }

  async updateListing(channelListingId: string, listing: CanonicalListing): Promise<{ updated: boolean; evidence: EvidenceRecord }> {
    return { updated: true, evidence: fakeEvidenceRecord(listing.tenantId, 'test_listing_updated', `[TEST-ONLY StockX] updated ${channelListingId}`, channelListingId, {}) };
  }

  async pauseListing(channelListingId: string): Promise<{ paused: boolean; evidence: EvidenceRecord }> {
    return { paused: true, evidence: fakeEvidenceRecord('', 'test_listing_paused', `[TEST-ONLY StockX] paused ${channelListingId}`, channelListingId, {}) };
  }

  async resumeListing(channelListingId: string): Promise<{ resumed: boolean; evidence: EvidenceRecord }> {
    return { resumed: true, evidence: fakeEvidenceRecord('', 'test_listing_resumed', `[TEST-ONLY StockX] resumed ${channelListingId}`, channelListingId, {}) };
  }

  async endListing(channelListingId: string): Promise<{ ended: boolean; evidence: EvidenceRecord }> {
    return { ended: true, evidence: fakeEvidenceRecord('', 'test_listing_ended', `[TEST-ONLY StockX] ended ${channelListingId}`, channelListingId, {}) };
  }
  ...
  async syncInventory(channelListingId: string, quantity: number): Promise<{ synced: boolean; evidence: EvidenceRecord }> {
    return { synced: true, evidence: fakeEvidenceRecord('', 'test_inventory_synced', `[TEST-ONLY StockX] synced qty=${quantity}`, channelListingId, { quantity }) };
  }

  async syncPrice(channelListingId: string, price: Money): Promise<{ synced: boolean; evidence: EvidenceRecord }> {
    return { synced: true, evidence: fakeEvidenceRecord('', 'test_price_synced', `[TEST-ONLY StockX] synced price=${price.amount}`, channelListingId, { price }) };
  }
  ...
  async respondToOffer(offerId: string, response: 'accept' | 'decline' | 'counter', counterAmount?: Money): Promise<{ responded: boolean; evidence: EvidenceRecord }> {
    ...
    return { responded: true, evidence: fakeEvidenceRecord('', 'test_offer_responded', `[TEST-ONLY StockX] responded`, offerId, {}) };
  }
  ...
  async acknowledgeOrder(channelOrderId: string): Promise<{ acknowledged: boolean; evidence: EvidenceRecord }> {
    return { acknowledged: true, evidence: fakeEvidenceRecord('', 'test_order_acknowledged', `[TEST-ONLY StockX] ack ${channelOrderId}`, channelOrderId, {}) };
  }

  async cancelOrder(channelOrderId: string, reason: string): Promise<{ cancelled: boolean; evidence: EvidenceRecord }> {
    return { cancelled: true, evidence: fakeEvidenceRecord('', 'test_order_cancelled', `[TEST-ONLY StockX] cancel ${channelOrderId}: ${reason}`, channelOrderId, { reason }) };
  }
  ...
  async verifyListing(channelListingId: string): Promise<{ verified: boolean; evidence: EvidenceRecord }> {
    return { verified: true, evidence: fakeEvidenceRecord('', 'test_listing_verified', `[TEST-ONLY StockX] verify ${channelListingId}`, channelListingId, {}) };
  }

  async verifyOrder(channelOrderId: string): Promise<{ verified: boolean; evidence: EvidenceRecord }> {
    return { verified: true, evidence: fakeEvidenceRecord('', 'test_order_verified', `[TEST-ONLY StockX] verify ${channelOrderId}`, channelOrderId, {}) };
  }

  async shutdown(): Promise<void> {}
}

export function createSTKXTestAdapter(): STKXTestAdapter {
  return new STKXTestAdapter();
}
