#!/usr/bin/env node
/**
 * Workflow M — Premium Discord Alert
 *
 * High-scoring opportunity + premium entitlement fixture → alert captured by local Discord adapter; public feed delayed.
 *
...
async function main() {
  ...
  console.log('=== Workflow M — Premium Discord Alert ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOPREMI' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'premium-discord-alert demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow M — Premium Discord Alert', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow M — Premium Discord Alert', hook: 'Demo hook',
