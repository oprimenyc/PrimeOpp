#!/usr/bin/env node
/**
 * Workflow A — Amazon Price Drop
 *
 * Ingest local Amazon fixture, normalize product, calculate effective price, compare history, score, create affiliate-ready publication, create local alert.
 *
...
async function main() {
  ...
  console.log('=== Workflow A — Amazon Price Drop ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOAMAZO' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'amazon-price-drop demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow A — Amazon Price Drop', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow A — Amazon Price Drop', hook: 'Demo hook',
