 ...
 * Workflow L — AMOS Job
 *
 * Verified deal → structured video and social content job created → expiration included → unsupported claims prohibited.
 *
...
async function main() {
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'amos-content-job demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow L — AMOS Job', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow L — AMOS Job', hook: 'Demo hook',
