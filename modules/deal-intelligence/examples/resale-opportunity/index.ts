#!/usr/bin/env node
/**
 * Workflow I — Resale Opportunity
 *
 * Low retailer price + marketplace comp fixture + fees and shipping calculated → BUY or PASS recommendation.
 *
...
async function main() {
  ...
  console.log('=== Workflow I — Resale Opportunity ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMORESAL' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'resale-opportunity demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow I — Resale Opportunity', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow I — Resale Opportunity', hook: 'Demo hook',
