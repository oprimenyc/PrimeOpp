...
async function main() {
  ...
  console.log('=== Workflow C — Lowe's Hidden Markdown ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOLOWES' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'lowes-hidden-markdown demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow C — Lowe's Hidden Markdown', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow C — Lowe's Hidden Markdown', hook: 'Demo hook',
