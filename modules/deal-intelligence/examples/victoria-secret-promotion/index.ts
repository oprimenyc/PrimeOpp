#!/usr/bin/env node
/**
 * Workflow F — Victoria's Secret Promotion
 *
 * Detect membership or coupon requirement, calculate effective price, insert affiliate disclosure, create content job.
 *
...
async function main() {
  ...
  console.log('=== Workflow F — Victoria's Secret Promotion ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOVICTO' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'victoria-secret-promotion demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow F — Victoria's Secret Promotion', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow F — Victoria's Secret Promotion', hook: 'Demo hook',
