#!/usr/bin/env node
/**
 * Workflow — Affiliate Publication
 *
 * Verified deal → affiliate link built with disclosure → publication captured by local adapter.
 *
...
async function main() {
  ...
  console.log('=== Workflow — Affiliate Publication ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOAFFIL' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'affiliate-publication demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow — Affiliate Publication', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow — Affiliate Publication', hook: 'Demo hook',
