#!/usr/bin/env node
/**
 * Workflow E — Bath & Body Works Stack
 *
 * Combine sale and coupon, detect stack uncertainty, require confirmation, publish as conditional.
 *
...
async function main() {
  ...
  console.log('=== Workflow E — Bath & Body Works Stack ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOBATHB' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'bath-body-works-stack demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow E — Bath & Body Works Stack', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow E — Bath & Body Works Stack', hook: 'Demo hook',
