#!/usr/bin/env node
/**
 * Workflow G — Regional Store Opportunity
 *
 * Ingest regional store fixture, score, alert regional subscribers.
 *
...
async function main() {
  ...
  console.log('=== Workflow G — Regional Store Opportunity ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOREGIO' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'regional-store-opportunity demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow G — Regional Store Opportunity', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow G — Regional Store Opportunity', hook: 'Demo hook',
