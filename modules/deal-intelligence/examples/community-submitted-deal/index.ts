...
async function main() {
  ...
  console.log('=== Workflow J — Community Submission ===');

  // Step 1: Ingest fixture observation
  const obs = sdk.ingestObservation({
    source: 'manual-entry',
    retailerId: 'ret:amazon',
    productIdentifier: { type: 'ASIN', value: 'B0DEMOCOMMU' },
  ...
  const prod = sdk.normalizeProduct({
    sourceTitle: 'community-submitted-deal demo product',
    brand: 'demo'
  });
  ...
  const alerts = await sdk.emitAlert({
    ...
    headline: 'Workflow J — Community Submission', body: 'Demo', score: s.overall.value
  });
  console.log('Alerts emitted:', alerts.length);

  // Step 8: AMOS job
  const amos = sdk.createAmosJob({
    kind: 'daily-top-deals', title: 'Workflow J — Community Submission', hook: 'Demo hook',
